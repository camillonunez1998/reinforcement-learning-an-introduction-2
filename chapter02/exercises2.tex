...
\begin{document}
    ...
    Note that, in the case of a random walk, even though constant step size used for action-value estimation achieves the best performance in terms of average reward received, but ironically performs the worse in terms of average optimal action! This says a lot, since basically it tells us that the value of actions, if not stationary, should not be... Aqui hay problemas.
